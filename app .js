...
function loadBookings(callback) {
  showLoading(true);
  apiGet({action: 'getBookings'}, function(err, data) {
    showLoading(false);
...
function saveBooking(booking, callback) {
  var url = GAS_URL + '?action=addBooking&data=' + encodeURIComponent(JSON.stringify(booking));
  fetch(url)
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (data.status === 'ok') {
...
function removeBooking(id, callback) {
  var url = GAS_URL + '?action=deleteBooking&id=' + encodeURIComponent(id);
  fetch(url)
    .then(function(r) { return r.json(); })
...
function switchTab(name) {
  ...
  if (name === 'rooms') {
    loadBookings(function() { updateRoomAvail(); });
  }
}

function updateRoomAvail() {
  // 本日の予約状況を部屋カードに反映（将来拡張用）
}
...
function buildTG() {
  ...
  S.bookings.forEach(function(bk) {
    if (bk.room === S.room && bk.date === S.date) {
      booked = booked.concat(Array.isArray(bk.times) ? bk.times : bk.times.split(','));
    }
  });
...
function confirmBooking() {
  ...
  saveBooking(booking, function(result) {
    ...
    t.textContent = result ? '\u2705 \u4e88\u7d04\u304c\u5b8c\u4e86\u3057\u307e\u3057\u305f\uff01' : '\u26a0\ufe0f \u4fdd\u5b58\u306b\u5931\u6557\u3057\u307e\u3057\u305f';
...
function renderCancelPanel() {
  ...
  ymKeys.forEach(function(ym, i) {
    ...
    months[ym].forEach(function(item) {
      var b = item.b;
      var realIdx = item.i;
      var times = Array.isArray(b.times) ? b.times : b.times.split(',');
...
function executeCancelBooking() {
  ...
  removeBooking(b.id, function(result) {
    ...
    renderCancelPanel();
    var t = document.getElementById('toast');
    t.textContent = result ? '\ud83d\uddd1 \u4e88\u7d04\u3092\u30ad\u30e3\u30f3\u30bb\u30eb\u3057\u307e\u3057\u305f' : '\u26a0\ufe0f \u30ad\u30e3\u30f3\u30bb\u30eb\u306b\u5931\u6557\u3057\u307e\u3057\u305f';
    t.classList.add('show');
...
function buildTL(dateStr) {
  ...
  TL_HOURS.forEach(function(h) {
    ...
    d.className = 'tl-h'; d.textContent = h;
    hcell.appendChild(d);
  });
  ...
  Object.keys(ROOMS).forEach(function(rk) {
    ...
    S.bookings.forEach(function(bk) {
      ...
      var times = Array.isArray(bk.times) ? bk.times : bk.times.split(',');
      var s = times.slice().sort();
