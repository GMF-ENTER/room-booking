...
var EMAILJS_TEMPLATE_ID = 'template_v4gj2pp';
var EMAILJS_PUBLIC_KEY  = 'gu8KQlUmSheVKctvX';
...
function sendConfirmEmail(booking) {
  if (typeof emailjs === 'undefined') return;
  var times = Array.isArray(booking.times) ? booking.times : booking.times.split(',');
  var sorted = times.map(function(t){ return t.trim(); }).sort();
  var timeEnd = addMinutes(sorted[sorted.length-1], 30);
  var roomName = ROOMS[booking.room] ? ROOMS[booking.room].name : booking.room;
  var coName = booking.co === 'hifive' ? 'HIGH FIVE' : 'HIGHWAY PLANET';
