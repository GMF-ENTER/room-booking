...
function buildTL(dateStr) {
  ...
  wrap.innerHTML = '';
  // 表示範囲：09:00〜19:00（30分刻み）
  var TL_START = 9 * 60;   // 09:00 を分に変換
  var TL_END   = 19 * 60;  // 19:00 を分に変換
  var TL_SPAN  = TL_END - TL_START; // 総分数 = 600分
  var TL_LABELS = ['09:00','10:00','11:00','12:00','13:00','14:00','15:00','16:00','17:00','18:00'];
  ...
  function pct(min) {
    return ((min - TL_START) / TL_SPAN * 100).toFixed(4) + '%';
  }
  ...
  TL_LABELS.forEach(function(h) {
    ...
    d.className = 'tl-h';
    d.textContent = h;
    // ラベルの時刻を分に変換して正確な位置に配置
    var labelMin = toMin(h);
    d.style.left = ((labelMin - TL_START) / TL_SPAN * 100).toFixed(4) + '%';
    hcell.appendChild(d);
  });
  ...
  Object.keys(ROOMS).forEach(function(rk) {
    ...
    S.bookings.forEach(function(bk) {
      ...
      var endMin   = toMin(times[times.length - 1]) + 30;

      var bl = document.createElement('div');
      bl.className = 'tl-block ' + (bk.co === 'hifive' ? 'tl-hf' : 'tl-hw');
      bl.style.left  = pct(startMin);
      bl.style.width = pct(endMin - startMin + TL_START).replace('%','') !== pct(startMin).replace('%','')
        ? ((endMin - startMin) / TL_SPAN * 100).toFixed(4) + '%'
        : '5%';
      // widthを直接計算
      bl.style.left  = ((startMin - TL_START) / TL_SPAN * 100).toFixed(4) + '%';
      bl.style.width = ((endMin   - startMin) / TL_SPAN * 100).toFixed(4) + '%';
